...
export default function StudentsAttendanceRecord() {
  ...
  useEffect(() => {
    // Note: undefined passes the default branch inside the hook
    fetchStudentAttendancesByBranch(undefined, currentPage, limit, debouncedSearch);
  }, [fetchStudentAttendancesByBranch, currentPage, limit, debouncedSearch]);
  ...
  // Helper function to render status badges dynamically & safely
  const renderStatusBadge = (record) => {
    if (!record) return <div className="badge badge-ghost font-medium px-4 py-3">Unknown</div>;

    // 1. If status is a string (e.g., "Present", "Absent")
    if (typeof record.status === 'string') {
      const statusFormatted = record.status.toLowerCase();
      ...
      return <div className="badge badge-ghost font-medium px-4 py-3 capitalize">{record.status}</div>;
    }

    // 2. If the API provides boolean fields for present/absent directly
    ...
    // 3. Fallback for boolean status (e.g., status: true from your API image)
    if (typeof record.status === 'boolean') {
      return (
        <div className={`badge ${record.status ? 'badge-info' : 'badge-error'} text-white font-medium px-4 py-3`}>
          {record.status ? 'Present' : 'Absent'}
        </div>
      );
    }

    return <div className="badge badge-ghost font-medium px-4 py-3 capitalize">Unknown</div>;
  };

  // --- Applying Client-Side Filters ---
  const filteredRecords = studentAttendances?.filter((record) => {
    const studentClass = record.studentClass || record.student?.studentClass;
    const section = record.section || record.student?.section;

    // Check Class filter
    if (appliedFilterClass && studentClass !== appliedFilterClass) {
      return false;
    }

    // Check Section filter
    if (appliedFilterSection && section !== appliedFilterSection) {
      return false;
    }
    ...
    <div className="p-4 md:p-8 max-w-7xl mx-auto font-sans relative">
      ...
      {error && (
        <div className="alert alert-error shadow-lg mb-6">
          ...
          <button 
            onClick={() => fetchStudentAttendancesByBranch(undefined, currentPage, limit, debouncedSearch)}
      ...
      <div className="bg-base-100 p-4 rounded-xl shadow-sm border border-base-200 mb-6 space-y-4">
        ...
        {/* Class & Section Dropdowns + Search Button */}
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={tableHeaders.length} className="py-12 text-center">
                    <MtableLoading data={filteredRecords} />
